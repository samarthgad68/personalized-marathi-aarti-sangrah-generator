...
const storage = multer.diskStorage({
  ...
  filename: (_req, file, cb) => {
    ...
    const uniqueName = `user_photo_${Date.now()}_${Math.round(
      Math.random() * 1e6
    )}${ext}`;
  ...
  if (photoUrl.includes('temp/uploads/')) {
    ...
    try {
      ...
      if (commaIdx !== -1) {
        ...
        if (buffer.length > 0) {
          const filename = `base64_photo_${Date.now()}_${Math.round(
            Math.random() * 1e6
          )}.${ext}`;
...
async function startServer() {
        ...
        if (err instanceof multer.MulterError) {
          if (err.code === 'LIMIT_FILE_SIZE') {
            return res.status(400).json({
              error:
                'फोटोची साईज लहान करा (Max 25MB).',
            });
          }

          return res.status(400).json({
            error: `अपलोड त्रुटी: ${err.message}`,
          });
        } else if (err) {
          return res.status(400).json({
            error:
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        // LIVE Razorpay configuration required
        if (!keyId || !keySecret) {
          return res.status(500).json({
            success: false,
            error:
              'Razorpay Live API keys configured नाहीत. AIC Cloud Environment Variables तपासा.',
          });
        }
        ...
        if (keyId.includes('your_key_id')) {
          return res.status(500).json({
            success: false,
            error:
              'Razorpay Key ID अजून placeholder आहे. Live Razorpay Key ID configure करा.',
          });
        }
        ...
        try {
          ...
          const order =
            await instance.orders.create({
              ...
              notes: {
                purpose:
                  'Personalized Marathi Aarti Sangrah PDF',
              },
            });
          ...
          console.error(
            'Razorpay LIVE order creation failed:',
            razorpayErr
          );
          ...
          return res.status(500).json({
            ...
            error:
              ...
              razorpayErr?.message ||
              'Razorpay Live Order तयार करता आला नाही.',
          });
        }
      } catch (error: any) {
        console.error(
        ...
        return res.status(500).json({
          error:
            error.message ||
            'पेमेंट ऑर्डर तयार करण्यात अडचण आली.',
        });
      }
    }
  );
  ...
  setInterval(() => {
    ...
    for (
      const [token, data] of paidSessions.entries()
    ) {
      if (
        now - data.createdAt >
        24 * 60 * 60 * 1000
  ...
  // HOME PAGE -> GENERATOR PAGE REDIRECT
  app.post(
      ...
      try {
        ...
        if (
          ...
          if (
            ...
            return res.status(400).json({
              error:
                'पेमेंट व्हॅलिडेशन अयशस्वी झाले.',
            });
          }
        }
        ...
        res.status(500).json({
          error:
            error.message ||
            'पेमेंट पडताळणीमध्ये त्रुटी आली.',
        });
      }
    }
  );

  // 5. GENERATE AND DOWNLOAD PDF ENDPOINT
  // GENERATOR PAGE
  app.post(
    '/api/pdf/generate',
    async (req, res) => {
      let pdfPath: string | null = null;
      let photoPathToDelete: string | null =
        null;
      ...
      try {
        ...
        if (!isValid) {
          return res.status(403).json({
            error:
              'पेमेंट सेशन वैध नाही. कृपया प्रथम ₹99 पेमेंट करा.',
          });
        }
        ...
        res.setHeader(
          'Content-Disposition',
          'attachment; filename="Personalized_Marathi_Aarti_Sangrah.pdf"'
        );
        ...
        res.status(500).json({
          error:
            error.message ||
            'PDF निर्मितीमध्ये त्रुटी आली.',
        });
      }
    }
  );

  // 6. VERIFY PAYMENT AND GENERATE PDF ENDPOINT
  // BACKWARD COMPATIBLE
  app.post(
    '/api/payment/verify',
    async (req, res) => {
      ...
      try {
        ...
        console.error(
          'PDF Generation / Payment Verification error:',
          error
        );
